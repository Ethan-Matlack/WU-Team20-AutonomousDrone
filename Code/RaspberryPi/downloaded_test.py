...
class AutopilotConnection:
    ...
    def __init__(self, device="/dev/ttyAMA0", baudrate=115200,
                 source_system=1, source_component=2, auto_reconnect=True, mavlink2=True):
        ...
        print(f"Connecting to {device}, baudrate: {baudrate}."
              f"Set to system: {source_system}, component: {source_component}")
    ...
    def connect(self):
        ...
        print(f"Connecting to target: {self.master.target_system}, component: {self.master.target_component}")
    ...
    def get_home(self, wait_condition=True):
        ...
        # Home point is created when the motors are armed, so it won't be correct before then
        ...
        while wait_condition:
            ...
            if msg_type == "HOME_POSITION":
                print(f"Got home position: {msg_data}")
                return msg_data
    ...
    def get_yaw(self, wait_condition=True):
        ...
        Warn: make sure you have run request_all() before doing this.
        ...
        while wait_condition:
            ...
            if msg_type == "ATTITUDE":
                print(f"Got attitude: {msg_data}")
                return msg_data["yaw"]  # -pi..pi

        # Didn't get it, i.e. wait_condition evaluated to False at some point
        return None

    def send_waypoints(self, waypoints, radius=0.5, land_at_end=True, origin=None, takeoff_at_beginning=True):
        """
        ...
        for i, (hold, lat, lon, alt) in enumerate(waypoints):
            ...
            if origin is not None:
                ...
                # TODO: Verify function od this line. Previously was only sending (orig_lon, lat, lon) params.
            ...
            if takeoff_at_beginning and i == 0:
                wp.add(mavutil.mavlink.MAVLink_mission_item_message(
                    ...
                    0, 0, 0, math.nan,
                    lat, lon, alt))
                print(f"Takeoff at: {lat}, {lon}, {alt}")
                seq += 1
            ...
            wp.add(mavutil.mavlink.MAVLink_mission_item_message(
                ...
                lat, lon, alt))
            print(f"Waypoint at: {lat}, {lon}, {alt}")
            seq += 1
    ...
    @staticmethod
    def get_location_meters(orig_lat, orig_lon, north_dist, east_dist):
        """
        ...
        d_lat = north_dist / earth_radius
        d_lon = east_dist / (earth_radius * math.cos(orig_lat * math.pi / 180))

        # New position in decimal degrees
        new_lat = orig_lat + (d_lat * 180 / math.pi)
        new_lon = orig_lon + (d_lon * 180 / math.pi)

        return new_lat, new_lon

    @staticmethod
    def get_yaw_from_quaternion(q, degrees=False):
        """
...
class AutopilotCommunicationReceive(multiprocessing.Process):
    """
    The process that receives information from the autopilot. This will manage enabling and disabling. If a certain
    channel is set high, then we'll start streaming video to the laptop. If it's set higher, then we'll initiate
    shutdown (though the exact functionality is in callbacks implemented in control.py). This also prints out debug
    information about if we have acquired the landing target.
    """

    def __init__(self, connection, channels=[6], cutoffs=[1282, 1716], rate=2, on_mode=None):
        """
        ...
        Cutoffs: my switch 6 goes between 1065, 1499, and 1933, and the defaults are between those
        ...
        while not self.exiting.is_set():
            ...
            if msg_type == "RC_CHANNELS_RAW" or msg_type == "RC_CHANNELS":
                ...
                if "position_valid" in msg_data:
                    ...
                    if debug:
                        print(f"Target acquired: {target_acquired}")
                else:
                    if debug:
                        print(f"Landing target: {msg_data}")

    def set_mode(self, channel, mode):
        if self.modes[channel] != mode:
            print(f"Changing channel: {channel} to mode: {mode}")
...
class AutopilotCommunicationSend(multiprocessing.Process):
    """
    The process that sends information to the autopilot. This will both get the home point, calculate the flight path,
    and send that to the autopilot. It will also send landing target messages whenever we detect the target in an image
    (see control.py that receives these bounding boxes and puts them into the buffer manager).
    """
    ...
    def run(self, enable_auto=True, min_num_detections=10):
        # If we will fly autonomously, then we first need to know where it was armed and from that we'll create the
        # flight plan. Request all data at a low frequency.
        #
        ...
        if enable_auto:
            ...
            # HOME_POSITION always sets the quaternion to (1,0,0,0) regardless of orientation, so get it from an
            # ATTITUDE packet now since we're armed. Note: don't need to request_all() in this process since we did
            # it in the reception process and these are both over the same link via mavlink-router.
            ...
            if home and yaw:
                ...
                # q is attitude quaternion: w, x, y, z order -- zero-rotation is {1, 0, 0, 0}
                # yaw = self.connection.get_yaw_from_quaternion(home["q"]) # from North

                # Calculate distances North and East to be a certain distance forward from the home position/heading
                # Note: This math works for northern hemisphere
                ...
                forward2 = (1, forward_11m[0], forward_11m[1], 4.5)  # forward 5.5 meters, hold 1s
    ...
    def send_detection(self, detection,
        ...
        # Above are all for camera orientation, but it's actually rotated on the frame
